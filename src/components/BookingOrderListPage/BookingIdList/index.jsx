...
class BookingIdList extends React.Component {
  ...
  render() {
    ...
    let id = 0
    if (this.props.bookOrder !== undefined) {
      id = this.props.bookOrder.id
    }
    return (
      <div className="booking-id-list">
        {orderList.map((item, index) => (
          <div className={item.id === id ? "active booking-order-wrapper" : "booking-order-wrapper"} key={index} onClick={() => { this.selectOrder(item) }}>
